...
const oauth = OAuth({
  consumer: { key: process.env.WLG_TV_CONSUMER_KEY, secret: process.env.WLG_TV_CONSUMER_SECRET },
  signature_method: "HMAC-SHA1",
  hash_function: (base, key) => crypto.createHmac("sha1", key).update(base).digest("base64"),
});
const token = { key: process.env.WLG_TV_ACCESS_TOKEN, secret: process.env.WLG_TV_ACCESS_TOKEN_SECRET };
...
function blockMissingProductData(product, state, poNumber, supplierName) {
...
async function main() {
  ...
  for (const po of pos) {
    ...
    for (const child of children) {
      // --------------------------------------------------
      // SAFETY CHECK — description + price required
      // --------------------------------------------------
      if (
      ...
      if (state[invKey]) {
        console.log(
          `    ${child.Code}: inventory already added for this PO — skipping`
        );
        continue;
      }

      const warehouseCode = await getOrAssignWarehouse(
        child.Code,
        state
      );

      const ok = await addInventory(
        child.Code,
        PRESALE_QTY,
        warehouseCode
      );
      ...
      if (!state[bomKey]) {
        const ok = await linkChildrenToParent(parent, children.map((c) => c.ProductID));
        if (ok) state[bomKey] = { done: true, date: new Date().toISOString(), supplier: po.Supplier?.Name || null };
      } else {
      ...
      if (!state[titleKey]) {
       ...
       if (ok) state[titleKey] = { done: true, date: new Date().toISOString(), supplier: po.Supplier?.Name || null };
      } else {
